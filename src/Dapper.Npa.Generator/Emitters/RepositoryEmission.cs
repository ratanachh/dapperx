...
internal static class RepositoryEmission
{
    ...
    public static void EmitConstructorExtras(StringBuilder sb, EntityModel entity, IReadOnlyDictionary<string, EntityModel> allModels, string provider)
    {
        ...
        {
            sb.AppendLine("    private readonly Dapper.Npa.Abstractions.Configuration.IDapperXOptions? _options;");
            sb.AppendLine();
        }
        ...
        if (hasAuditing)
        {
            sb.AppendLine("    private readonly Dapper.Npa.Abstractions.Auditing.IAuditingProvider? _auditingProvider;");
            sb.AppendLine();
        }

        if (entity.TenantIdColumn is not null)
        {
            sb.AppendLine("    private readonly Dapper.Npa.Abstractions.Tenancy.ITenantProvider? _tenantProvider;");
            sb.AppendLine();
        }

        if (entity.Sequence is not null)
        {
            sb.AppendLine("    private readonly Dapper.Npa.Abstractions.Sequences.ISequenceAllocator? _sequenceAllocator;");
            sb.AppendLine();
        }
    ...
    private static void EmitBatchTrace(StringBuilder sb, string sqlProperty)
        => sb.AppendLine($"        Dapper.Npa.Runtime.Logging.SqlExecutionLogger.TryLogBatchTrace(DbExecutor.CreateLogContext(MethodName, Options, Provider), {sqlProperty}, list.Count, effectiveBatchSize);");
    ...
    private static void EmitBulkCapableAssignedInsertMany(StringBuilder sb, EntityModel entity, string entityFqn, string provider)
    {
        ...
        sb.AppendLine("            await " + executorType + ".Instance.InsertAsync(new global::Dapper.Npa.Provider.Common.BulkInsertContext");
        sb.AppendLine("            {");
    ...
    private static void EmitUpdateMany(StringBuilder sb, EntityModel entity, string entityFqn)
    {
        ...
        if (idProp is not null)
        {
            ...
            sb.AppendLine("            throw new Dapper.Npa.Abstractions.Exceptions.ConcurrencyException(\"One or more updates failed due to optimistic concurrency conflicts.\", conflictingKeys);");
        }
        if (entity.HasBatchLifecycle)
    ...
    private static void EmitDeleteMany(StringBuilder sb, EntityModel entity, string entityFqn)
    {
        ...
        {
            sb.AppendLine("        if (totalAffected != totalAttempted)");
            sb.AppendLine("            throw new Dapper.Npa.Abstractions.Exceptions.ConcurrencyException(\"One or more deletes failed due to optimistic concurrency conflicts.\", conflictingKeys);");
        }
        if (entity.HasBatchLifecycle)
