...
internal static class GraphGenerator
{
        ...
        IReadOnlyList<RelationshipModel> manyToManyRels)
    {
        ...
        sb.AppendLine("        Dapper.Npa.Runtime.Logging.SqlExecutionLogger.TryLogBatchTrace(DbExecutor.CreateLogContext(MethodName, Options, Provider), InsertSql, 1, 1);");
        ...
        IReadOnlyList<RelationshipModel> manyToManyRels)
    {
        ...
        sb.AppendLine("        Dapper.Npa.Runtime.Logging.SqlExecutionLogger.TryLogBatchTrace(DbExecutor.CreateLogContext(MethodName, Options, Provider), UpdateSql, 1, 1);");
        sb.AppendLine("        var ownsTransaction = transaction is null;");
        sb.AppendLine("        transaction ??= _connection.BeginTransaction();");
        sb.AppendLine("        try");
        sb.AppendLine("        {");
        ...
        IReadOnlyList<RelationshipModel> manyToManyRels)
    {
        ...
        sb.AppendLine("        Dapper.Npa.Runtime.Logging.SqlExecutionLogger.TryLogBatchTrace(DbExecutor.CreateLogContext(MethodName, Options, Provider), DeleteSql, 1, 1);");
        sb.AppendLine("        var ownsTransaction = transaction is null;");
        sb.AppendLine("        transaction ??= _connection.BeginTransaction();");
        sb.AppendLine("        try");
        sb.AppendLine("        {");
