        ...
        IReadOnlyDictionary<string, EntityModel> allModels)
    {
        ...
        foreach (var rel in lazyRels)
        {
            ...
            if (rel.Kind == "ManyToOne" && rel.IsLazyReference)
            {
                ...
                sb.AppendLine($"        entity.{rel.PropertyName} = new Dapper.Npa.Relations.Lazy.LazyReference<{targetFqn}>((conn, tx) =>");
            ...
            if (rel.IsLazyMap)
            {
                ...
                sb.AppendLine($"        entity.{rel.PropertyName} = new Dapper.Npa.Relations.Lazy.LazyMap<{mapKeyType}, {targetFqn}>(r => r.{mapKeyProp}!, async (conn, tx) =>");
                sb.AppendLine("        {");
            ...
            sb.AppendLine($"        entity.{rel.PropertyName} = new Dapper.Npa.Relations.Lazy.LazyCollection<{targetFqn}>(async (conn, tx) =>");
            sb.AppendLine("        {");
            EmitQueryRowsAssignment(sb, targetModel, targetFqn, sqlName, idProp.PropertyName);
            sb.AppendLine("            return rows;");
            sb.AppendLine("        });");
        ...
        {
            sb.AppendLine($"        entity.{ec.PropertyName} = new Dapper.Npa.Relations.Lazy.LazyCollection<{ec.ElementTypeName}>(async (conn, tx) =>");
