...
internal static class CpqlSemanticValidator
{
            ...
            if (!semantic.ReferencedCtes.Contains(cteName))
            {
                ctx.ReportDiagnostic(Diagnostic.Create(
                    new DiagnosticDescriptor("DPXCPQL030", "Unreferenced CTE",
                        "CTE '{0}' is never referenced in the query", "Dapper.Npa.CPQL",
    ...
    {
        ctx.ReportDiagnostic(Diagnostic.Create(
            new DiagnosticDescriptor(id, "CPQL validation", msg, "Dapper.Npa.CPQL", severity, true), loc));
    ...
    {
        ctx.ReportDiagnostic(Diagnostic.Create(
            new DiagnosticDescriptor(id, "CPQL validation", msg, "Dapper.Npa.CPQL",
