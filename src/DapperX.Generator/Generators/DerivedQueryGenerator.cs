...
internal static class DerivedQueryGenerator
{
    ...
    private static void EmitLockSuffixAppend(StringBuilder sb, string provider)
    {
        ...
        {
            sb.AppendLine("        if (lockMode is DapperX.Core.Enums.LockMode.Pessimistic or DapperX.Core.Enums.LockMode.PessimisticRead)");
            sb.AppendLine("        {");
            sb.AppendLine("            var lockHint = lockMode switch");
            sb.AppendLine("            {");
            sb.AppendLine("                DapperX.Core.Enums.LockMode.Pessimistic => \"WITH (UPDLOCK, ROWLOCK)\",");
            sb.AppendLine("                DapperX.Core.Enums.LockMode.PessimisticRead => \"WITH (HOLDLOCK, ROWLOCK)\",");
            sb.AppendLine("                _ => \"\",");
            sb.AppendLine("            };");
            sb.AppendLine("            sql = DapperX.Runtime.Query.SqlServerTableHint.Apply(sql, lockHint);");
            sb.AppendLine("        }");
            return;
        }
        ...
        if (provider == "Sqlite")
        {
            sb.AppendLine("            DapperX.Core.Enums.LockMode.Pessimistic => throw new NotSupportedException(\"Pessimistic lock is not supported on SQLite.\"),");
            sb.AppendLine("            DapperX.Core.Enums.LockMode.PessimisticRead => throw new NotSupportedException(\"Pessimistic read lock is not supported on SQLite.\"),");
        }
        else
        {
            ...
            sb.AppendLine($"            DapperX.Core.Enums.LockMode.Pessimistic => \"{pessimistic}\",");
            sb.AppendLine($"            DapperX.Core.Enums.LockMode.PessimisticRead => \"{pessimisticRead}\",");
        }
        sb.AppendLine("            _ => \"\",");
        sb.AppendLine("        };");
    }
