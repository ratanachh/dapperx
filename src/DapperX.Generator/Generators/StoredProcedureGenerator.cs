...
internal static class StoredProcedureGenerator
{
        ...
        string provider)
    {
        ...
        if (model.ReturnKind == StoredProcedureReturnKind.ProcResult1)
        {
            ...
            sb.AppendLine($"        return new DapperX.Abstractions.StoredProcedures.ProcResult<{model.ProcResultTypeArguments[0]}>({SanitizeVarName(primary)}, __outputs);");
        }
        else
        {
            sb.AppendLine($"        return new DapperX.Abstractions.StoredProcedures.ProcResult<{model.ProcResultTypeArguments[0]}, {model.ProcResultTypeArguments[1]}>({SanitizeVarName(model.OutParameterNames[0])}, {SanitizeVarName(model.OutParameterNames[1])}, __outputs);");
        }
        ...
        string provider)
    {
        ...
        sb.AppendLine($"        return new DapperX.Abstractions.StoredProcedures.MultiResult<{t1}, {t2}>(__first, __second);");
        sb.AppendLine("    }");
        return sb.ToString();
    }
