...
public class LockingGenerationTests
{
    ...
    {
        var parameterTypes = new[] { typeof(string), typeof(Dapper.Npa.Core.Enums.LockMode) };
    ...
    public void ProductRepositoryImpl_FindByNameLocked_uses_SqlServerTableHint_for_sqlserver()
    {
        ...
        Assert.Contains("FindByNameLockedAsync(string name, Dapper.Npa.Core.Enums.LockMode lockMode)", source);
        Assert.Contains("Dapper.Npa.Runtime.Query.SqlServerTableHint.Apply(sql, lockHint)", source);
