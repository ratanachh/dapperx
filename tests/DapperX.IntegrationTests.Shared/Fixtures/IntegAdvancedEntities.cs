...
public interface IIntegProcOrderRepository : IRepository<IntegProcOrder, int>
{
    ...
    Task<DapperX.Abstractions.StoredProcedures.ProcResult<int, string>> ProcessOrderSpAsync(int orderId, decimal total);

    [StoredProcedure("sp_integ_proc_order_report",
        ResultSets = [typeof(IntegProcOrderSummary), typeof(IntegProcOrderLine)])]
    Task<DapperX.Abstractions.StoredProcedures.MultiResult<IntegProcOrderSummary, IntegProcOrderLine>> GetOrderReportSpAsync(int orderId);
}
#endif
